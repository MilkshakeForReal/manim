...
class Mobius_Strip(SpecialThreeDScene):
    ...
    def construct(self):
        ...
        h = 0.08
        ball_path = lambda t: R * np.array(
            [np.cos(t), np.sin(t), 0] + h * np.sin(-t / 2) * np.array([np.cos(t), np.sin(t), 0]) + h * np.cos(
                t / 2) * OUT)
...
class Mobius_Strip_heartshape(SpecialThreeDScene):
    ...
    def construct(self):
        ...
        heart_curve_func = lambda t: (16 * np.sin(t) ** 3 * RIGHT + (
                    13 * np.cos(t) - 5 * np.cos(2 * t) - 3 * np.cos(3 * t)
        ...
        heart_curve = ParametricFunction(heart_curve_func, t_min=-PI, t_max=PI, color=RED, stroke_width=10,
                                         stroke_opacity=0.9)
...
class Double_heart(SpecialThreeDScene):
    ...
    def construct(self):
        ...
        heart_curve_func = lambda t: (16 * np.sin(t) ** 3 * RIGHT + (
                    13 * np.cos(t) - 5 * np.cos(2 * t) - 3 * np.cos(3 * t) - np.cos(4 * t)) * UP + np.sin(t) * (
                                                  1 - abs(-t / PI)) ** 2 * 15 * OUT) * 0.2
        heart_curve_func_02 = lambda t: (16 * np.sin(t) ** 3 * RIGHT + (
                    13 * np.cos(t) - 5 * np.cos(2 * t) - 3 * np.cos(3 * t) - np.cos(4 * t)) * UP + np.sin(-t) * (
                                                     1 - abs(-t / PI)) ** 2 * 15 * OUT) * 0.2
        ...
        heart_shape_mobius_02 = ParametricSurface(
            lambda u, v: heart_curve_func_02(u) + v * np.cos(u / 2) * np.array([np.cos(u), np.sin(u), 0])
                         + v * np.sin(-u / 2) * OUT,
            u_min=-PI, u_max=PI, v_min=-r, v_max=r, resolution=(80, 12),
            checkerboard_colors=None, stroke_color=RED, stroke_opacity=0.8,
            stroke_width=0.8, fill_color=average_color(RED, PINK), fill_opacity=0.9)
...
class Spiral_Curve_test(Scene):
    ...
    def construct(self):
        ...
        spiral = ParametricFunction(
            lambda t: r0 * complex_to_R3(F_int(t) * np.exp(1j * PI / 2 * (t - 2)) + sum(P_minus_Q[1:int(t)])),
            t_min=1, t_max=1.0001, color=RED, stroke_width=2., stroke_opacity=1, step_size=0.01)

        def update_spiral(sp, dt):
            self.mytime += 1 / 30 * 2
            sp.become(ParametricFunction(
                lambda t: r0 * complex_to_R3(F_int(t) * np.exp(1j * PI / 2 * (t - 2)) + sum(P_minus_Q[1:int(t)])),
