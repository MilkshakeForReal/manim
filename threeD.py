...
class SquareSurface(ParametricSurface):
...
class Cylinder(ParametricSurface):
    ...
    def func(self, u, v):
        return np.array([np.cos(TAU * v), np.sin(TAU * v), 2 * (1 - u)])

class Test(ThreeDScene):

    def construct(self):
        self.set_camera_orientation() #no axes
        ...
        self.begin_ambient_camera_rotation(rate = 0.2)
        self.play(ReplacementTransform(surface, cylinder))
        self.wait()
