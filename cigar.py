...
class AM_GM(Scene):
    ...
    def construct(self):
        ...
        get_M = lambda theta: r * np.cos(theta * 2) * LEFT + r * np.sin(theta * 2) * UP + A #+a定位牛逼啊
        ...
        circle = Circle(radius=r, color=BLUE).move_to(A) #直接移到A位置
        ...
        line_color = WHITE
        BC = Line(B, C, color=line_color) #由点画线
        ...
        GM_bold = Line(G, M, color=bold_color[1], stroke_width=bold_stroke, plot_depth=0.6) #粗线，高图层
        ...
        self.wait(0.5)
        self.play(ShowCreation(BC), ShowCreation(dot_B), ShowCreation(dot_C)) #直径
        self.play(ShowCreation(BM), ShowCreation(CM), ShowCreation(AM))
        self.play(FadeIn(dot_M), Write(tex_M)) #打点，标记
        self.play(ShowCreation(GM))
        self.play(FadeIn(dot_G), Write(tex_G), FadeIn(angle_G))#自己写的角
        self.wait()
